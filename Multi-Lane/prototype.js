...
const CanvasQteTest = (() => {
  ...
  // 類型 B：戰術牌（留場上，結算後才消失）
  const CARD_TYPES = {
    accel:       { type:"accel",       cardClass:"action", name:"加速",   cost:0, speedValue:2, note:"速度 +2" },
  ...
  const STAGE5_CIRCUITS = [
    { id:"c1", name:"正常 3 道", icon:"🛣", lanes:3, bendCurve:0,    roadWidthScale:1.0, laneBonuses:null, sandLevel:1, hint:"標準賽道，無沙暴" },
    { id:"c2", name:"狹窄 1 道", icon:"▮",  lanes:1, bendCurve:0,    roadWidthScale:0.42,laneBonuses:null, sandLevel:2, hint:"強制 QTE 超車｜手牌 1 張盲牌" },
    { id:"c3", name:"彎道 2 道", icon:"↪",  lanes:2, bendCurve:0.18, roadWidthScale:1.0,
      laneBonuses:[{lane:0,speedMult:1.25,qteDiff:"hard",label:"內彎 ×1.25 / QTE 難"},
                   {lane:1,speedMult:0.9, qteDiff:"easy",label:"外彎 ×0.9 / QTE 易"}],
      sandLevel:3, hint:"內彎快但 QTE 難｜手牌 50% 盲牌" },
    { id:"c4", name:"正常 3 道", icon:"🛣", lanes:3, bendCurve:0,    roadWidthScale:1.0, laneBonuses:null, sandLevel:3, hint:"沙暴猛烈｜手牌 70% 盲牌" },
    // Boss 戰專用循環
  ...
  const STAGE5_TEAM_CARDS = {
    engineer: { type:"engineer", cardClass:"team", name:"工程師調校", cost:0, note:"動力上限 +1（永久）",        effect:"energyMaxPlus",  value:1,   persistence:"permanent",   persistenceLabel:"永久" },
    ...
    bigData:  { type:"bigData",  cardClass:"team", name:"大數據預測", cost:0, note:"顯示對手下一動作",            effect:"showOpponent",   value:1,   persistence:"permanent",   persistenceLabel:"永久" },
    ...
    cool:     { type:"cool",     cardClass:"team", name:"冷靜!",      cost:0, note:"QTE 容錯範圍 +20%",            effect:"qteForgiveness", value:0.2, persistence:"permanent",   persistenceLabel:"永久" },
  };
  ...
  function assignSandObscuredCards() {
    ...
    // 舊欄位保留為空集合（向下相容 isCostObscuredNow 的呼叫者）
    s5.obscuredCardIds = new Set();
    if (s5.bossStage) return;  // Boss 戰另一套機制
    const circ = currentCircuit();
    if (!circ) return;
    const handSize = app.hand.length;
    let count = 0;
    switch (circ.id) {
      case "c1": count = 0; break;
      case "c2": count = 1; break;
      case "c3": count = Math.floor(handSize * 0.5); break;
      case "c4": count = Math.floor(handSize * 0.7); break;
      default:   count = 0;
    }
    if (count <= 0) return;
    // 隨機選 count 張變盲牌（c2 手牌 < 1 時自然就是 0，不會出事）
    ...
    for (let i = 0; i < pickN; i++) {
      ...
      if (card && card.id) s5.blindCardIds.add(card.id);
    }
  }
  ...
  function initStage5State() {
    app.stage5 = {
      ...
      obscuredCardIds: new Set(),  // (舊欄位，保留為空集合，向下相容)
      blindCardIds: new Set(),     // 本回合被沙暴鎖為盲牌的卡 id（c2/c3/c4 用，回合開始時固定）
    };
    // 第一回合抽當前對手
    ...
    for (let i=0; i<laneCount; i++) {
      ...
      if (app.drag && i !== app.playerLane && droppable) {
        ...
        if (hovering) {
          ...
          text("棄牌換道", x+laneW/2, y+laneH/2+18, 12, "rgba(255,210,140,0.85)", "800", "center");
        } else {
          text("拖到此處 ‧ 棄牌換道", x+laneW/2, y+laneH-18, 11, "rgba(180,170,130,0.62)", "700", "center");
        }
      }
  ...
  // 盲牌邏輯：
  //   - Boss 戰中、blindPlaysRemaining > 0 時，所有手牌都是盲牌（打完 N 張後解除）
  //   - 一般 c2/c3/c4 賽段：用 blindCardIds 鎖定固定幾張為盲牌，整回合不變
  function isStage5BlindCard(card) {
    if (!isStage5() || !app.stage5) return false;
    if (!card) return false;
    const s5 = app.stage5;
    // Boss 戰：全手牌盲（破綻後解除）
    if (s5.bossStage) {
      if (s5.bossBroken) return false;
      if (s5.blindPlaysRemaining <= 0) return false;
      if (app.hand.indexOf(card) < 0) return false;
      return true;
    }
    // 一般賽段：依 blindCardIds 鎖定
    if (s5.blindCardIds && card.id && s5.blindCardIds.has(card.id)) return true;
    return false;
  }
  ...
  function drawStage5TeamCardTooltip(time) {
    ...
    // 直接進場的永久型車隊牌不顯示 COST
    const isDirectTeam = (c.cardClass === "team" && c.persistence === "permanent");
    if (!isDirectTeam) {
