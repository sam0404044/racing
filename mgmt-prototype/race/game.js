...
// ─── 音樂 ────────────────────────────────────────────────────────────────
const NORMAL_STAGE_BGM_SRC = "../assets/audio/normal-stage.wav";
...
// 路徑：../assets/BOSS-NCC7/
// 1x1 透明 PNG（給未使用的圖層 slot 用、避免 404 雜訊）
const _EMPTY_IMG = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=";

const NCC7_ENTRANCE_LAYER_SRCS = [
  "../assets/BOSS-NCC7/ncc7-intro-banner.jpg",      // 0 霓虹斜旗
  "../assets/BOSS-NCC7/ncc7-intro-title-code.png",  // 1 NCC-7 / CODE: 07 標題（PNG 去背）
  _EMPTY_IMG,                                       // 2 (舊 quote-panel、改成 runtime 純文字、無圖)
  "../assets/BOSS-NCC7/ncc7-intro-data-sweep.png",  // 3 資料流刷掠（PNG 去背）
  _EMPTY_IMG,                                       // 4 (舊 bottom-panel、被立繪蓋掉、不畫)
  _EMPTY_IMG,                                       // 5 (舊 stats-bars、改由 drawNcc7EntranceTypography 程式畫)
...
function spendTire(n) {
  ...
  if (app.tires === 0 && isStage2()) stage2OnTireOut();
}
// 守門：在「會切 mode 的結算流程」開始前，先確認輪胎沒爆
// 若已爆 → 強制進 tire-out 並回傳 true（呼叫端應 return）
function enforceTireOutIfDead() {
  if (isStage2() && app.tires === 0) {
    if (isQuali()) { qualiEnd("tireOut"); return true; }  // 排位：走排位結束、不進 1v1 DNF
    app.mode = "stage2-tire-out";
    return true;
  }
  return false;
}
...
function tickSpeedPopGates() {
  ...
  // （排位玩家被刷的淘汰動畫進行中時先不解鎖，等動畫播完 qualiEnd 才放行）
  if (app.inputLocked && !(app.quali && app.quali.pendingEnd)) app.inputLocked = false;
}
...
function calcOpponentSpeedAtLane(laneIdx, bypassAura = false) {
  const b = getLaneBonusFor(laneIdx, "opponent", bypassAura);
  // c6 油污 forceCornerQte 道：AI 用期望值（50% 吃加成 / 50% 滑到鄰道吃鄰道平均）
...
// c6 油污：對手在 forceCornerQte 道時、50% 失敗判定（第三關移植）
//   - 成功（50%）：留在當前道、正常吃加成
//   - 失敗（50%）：滑到隨機鄰道、之後結算用該鄰道加成
//   - B 清道夫被動：他被自己的光環保護、不會在油污失控
function handleOpponentForceQte() {
  ...
  if (Math.random() < 0.5) {
    ...
    if (safeLanes.length > 0) {
      const to = safeLanes[Math.floor(Math.random() * safeLanes.length)];
      app.opponentLane = to;
      app.opponentLaneVisual = to;
...
function resolveOpponentCircuit() {
  if (!isStage2()) return;
  if (isQuali()) return;
  // c6 油污：對手在 forceCornerQte 道、先做 50% 滑開判定
...
function getCardEffectiveSpeed(card) {
  ...
  // tirePreservation（保胎策略）：所有指令牌 -10（第三關移植）
  if (s2.teamCardsActive) {
...
function drawTutorialOverlay(time) {
  ...
  // 跳過教學（卡片右上角、每一步常駐）
  button("tutorial-skip", "跳過教學", cardX + cardW - 96, cardY + 8, 88, 26, false, "primary");
  // 標題
...
// 推進教學一步（按繼續鈕或事件觸發呼叫）
// 跳過教學：把步驟跳到最後、走 tutorialAdvance 的自然結束出口
//（active=false ＋ 洗牌賽段順序、與教學跑完同一條路徑）
function skipTutorial() {
  const t = app.stage2?.tutorial;
  if (!t || !t.active) return;
  t.stepIndex = TUTORIAL_STEPS.length - 1;
  tutorialAdvance();
  app.message = "已跳過教學";
}
...
// 取得下一賽道設定（用於右上預告）
// 重新隨機這個賽段的災害位置（第三關移植：c7 坑洞 / c6 油污 / sprite y 進度）
//   - c7 randomPothole：抽 1 安全道、其他都是坑
//   - c6 forceCornerQte：隨機選一道掛旗標、其他道清掉
function rerollCircuitHazards(circ) {
  if (!app.stage2 || !circ) return;
  const s2 = app.stage2;
  if (circ.randomPothole) {
    ...
    s2.potholeLanes = pits;
  } else {
    s2.potholeLanes = null;
  }
  if (circ.laneBonuses && circ.laneBonuses.some(b => b.forceCornerQte)) {
    const originalCirc = STAGE2_CIRCUITS.find(c => c.id === circ.id);
    const template = originalCirc?.laneBonuses?.find(b => b.forceCornerQte);
    if (template) {
      const targetLane = Math.floor(Math.random() * circ.lanes);
      const fallback = originalCirc.laneBonuses.find(b => !b.forceCornerQte) || {
        add: -10, mult: 1, label: "外緣 -10（安全）"
      };
      circ.laneBonuses = [];
      for (let i = 0; i < circ.lanes; i++) {
        if (i === targetLane) {
          circ.laneBonuses.push({ ...template, lane: i });
        } else {
          circ.laneBonuses.push({ ...fallback, lane: i });
        }
      }
  ...
  if (hasHazard) {
    s2.hazardSpriteProgress = 0.15 + Math.random() * 0.30;
  } else {
    s2.hazardSpriteProgress = null;
  }
}
...
function applyCircuit(circ) {
  ...
  // 同賽道模式（排位 / 正賽：賽段順序固定、跨圈回到同段）→ 佈局凍結：
  //   c8 車道值、坑洞/油污位置、災害 sprite、c8 揭曉集合都按 circuitIndex 凍結、第二圈沿用。
  const _sameTrack = !!(app.stage2 && (isQuali() || (SESSION && SESSION.circuitOrder)));
  const _idx = app.stage2 ? app.stage2.circuitIndex : null;
  const _ft = _sameTrack ? (app.stage2.frozenTrack = app.stage2.frozenTrack || {}) : null;
  const _frozen = _ft ? _ft[_idx] : null;
  if (_frozen) {
    // ── 還原這條賽道的固定佈局 ──
    circ.laneBonuses = _frozen.laneBonuses;
    app.laneBonuses = circ.laneBonuses;
    app.laneBonus = null;
    app.stage2.potholeLanes = _frozen.potholeLanes;
    app.stage2.hazardSpriteProgress = _frozen.hazardSpriteProgress;
    app.stage2.revealedC8Lanes = _frozen.revealedC8Lanes;  // 揭曉跨圈持久
  } else {
    // ── 第一次經過：生成佈局（同賽道模式則凍結起來）──
    if (circ.laneBonusDistribution) {
      circ.laneBonuses = generateLaneBonusesFromDistribution(circ);
    }
    app.laneBonuses = circ.laneBonuses;
    app.laneBonus = null;
    rerollCircuitHazards(circ);
    if (app.stage2) app.stage2.revealedC8Lanes = new Set();
    if (_ft) {
      _ft[_idx] = {
        laneBonuses: circ.laneBonuses ? circ.laneBonuses.map(b => ({ ...b })) : circ.laneBonuses,
        potholeLanes: app.stage2.potholeLanes ? [...app.stage2.potholeLanes] : null,
        hazardSpriteProgress: app.stage2.hazardSpriteProgress,
        revealedC8Lanes: app.stage2.revealedC8Lanes,  // 同一引用、跨圈累積揭曉
      };
    }
  }
  // 玩家若在新賽道沒有的道上 → 移到 0
...
function reshuffleNormalCircuitOrder() {
  const s2 = app.stage2;
  if (!s2) return;
  // 週末同一條賽道：session 帶了固定賽段順序 → 沿用、不洗牌
  if (SESSION?.circuitOrder && SESSION.circuitOrder.length) {
    s2.circuitOrder = [...SESSION.circuitOrder, ...SESSION.circuitOrder];
    return;
  }
...
function advanceCircuitOnCard() {
  ...
  // 套完 mult 才能正確判斷是否超速
  checkBendSpeedLimit();
  // c7 坑洞:結算後人在坑洞道 → 玩家 -1 胎、對手降速一半（第三關移植）
  checkPotholeDamage();
  ...
  if (s2.circuitStepsLeft > 0) {
    // 還沒走完當前賽段、停留
    // 多賽段內每動作要重新隨機災害位置（坑洞 / 油污、第三關移植）：
    //   有 QTE 在跑 → 設 flag、QTE 結束才重抽（避免拖牌時看舊位置、結算用新位置）
    // 同賽道模式（排位 / 正賽：賽段佈局凍結）→ 不重抽、坑洞/油污整場固定
    const _sameTrack = isQuali() || !!(SESSION && SESSION.circuitOrder);
    if (!_sameTrack && (currentCircuit()?.randomPothole ||
        currentCircuit()?.laneBonuses?.some(b => b.forceCornerQte))) {
      if (app.mode === "bend-qte" || app.mode === "splash-bend" || app._pendingBendQteTrigger) {
        s2._pendingHazardReroll = true;
      } else {
        rerollCircuitHazards(currentCircuit());
      }
    }
    return;
  }
...
function advanceCircuitToNextSegment() {
  ...
  s2.circuitIndex = order[nextIdx];
  applyCircuit(STAGE2_CIRCUITS[s2.circuitIndex]);
  // 排位：每段 1 動作、切段即完成一段 → 餵榜、計圈、判結束
  if (isQuali()) {
    s2.circuitStepsLeft = 1;
    qualiAfterSegment();
  }
}
// c7 坑洞段（第三關移植）：結算後檢查玩家／對手是否踩到坑洞道
//   玩家：-1 輪胎（補丁車隊牌可抵一次）；對手：速度 / 2
//   清道夫(B)光環互動：站光環道踩坑 → 不扣胎/不降速
function checkPotholeDamage() {
  const s2 = app.stage2;
  if (!s2 || !s2.potholeLanes || !s2.potholeLanes.length) return;
  const pits = s2.potholeLanes;
  ...
  if (playerHit && !playerSafeAura) {
    const patchIdx = s2.teamCardsActive.findIndex(c => c.effect === "savePothole");
    if (patchIdx >= 0) {
      s2.teamCardsActive.splice(patchIdx, 1);
      playerSafePatch = true;
    }
  }
  const playerSafe = playerSafeAura || playerSafePatch;
  const oppHit  = !isQuali() && pits.includes(app.opponentLane);  // 排位無對手、不判對手坑洞（避免虛擬對手在 0 道誤觸 -20 飛字）
  const oppSafe = oppHit && auraActive;
  ...
  if (oppHit && !oppSafe) {
    ...
    // 對手坑洞降速照舊、但不跳「-XX 坑洞」速度飛字（避免與玩家自己的坑洞飛字混淆）
...
function checkBendSpeedLimit() {
  ...
  const b = getLaneBonusFor(app.playerLane);
  // c6 油污：強制彎道 QTE（不看速度、踏入就觸發；第三關移植）
  if (b && b.forceCornerQte) {
    // 清道夫光環抵消：對手是清道夫時、玩家完全免疫強制 QTE
    if (isOpponentAuraActive()) {
      pushSpeedPop("player", "🛡️ 光環抵消油污", "#6fe0d0");
      app.message = "🛡️ 清道夫光環抵消油污！你安然通過";
      return;
    }
...
function initStage2State() {
  ...
  app.stage2 = {
    ...
    ahead: (SESSION?.lineup ? [...SESSION.lineup] : ["BOSS","C","B","A","P"]),
    passed: [],
    behind: (SESSION?.behind ? [...SESSION.behind] : []),
    ...
    potholeLanes: null,          // c7 坑洞段：本次抽到的坑洞道陣列（rerollCircuitHazards 動態設定）
    hazardSpriteProgress: null,  // c6/c7 sprite 在賽道路面上的 y 進度（0=horizon 1=玩家車）
    _pendingHazardReroll: false, // QTE 在跑時延後災害重抽旗標
    tirePreserveUsedThisRound: false,  // 保胎策略：本回合已用掉免疫
    deckBase: makeStage2InitialDeck(),
    deckPermanent: (SESSION?.deckPermanent ? SESSION.deckPermanent.map(c => ({...c})) : []),
    ...
    maxRounds: (SESSION?.maxRounds ?? 15),  // 終點線回合數（session 可覆寫）
    // 教學版專用旗標
    firstRoundReady: true,   // 第一回合不執行 advanceCircuit（讓玩家直接玩 init 設好的 c3）
    tutorial: {
      active: SESSION ? !!SESSION.tutorial : true,
      stepIndex: 0,
  ...
  // 聯賽模式：非教學賽段不走全 c3、直接洗正常賽段池（教學維持原行為、由腳本注入彎道）
  if (SESSION && !SESSION.tutorial) {
    reshuffleNormalCircuitOrder();
    const __firstIdx = app.stage2.circuitOrder[0];
    app.stage2.circuitIndex = __firstIdx;
    applyCircuit(STAGE2_CIRCUITS[__firstIdx]);
  }
...
function loadStage(idx) {
  ...
  // 聯賽模式：以排位起跑格覆寫名次（rankTotal 含未模擬的後方車手）
  if (SESSION?.rankTotal) app.rankTotal = SESSION.rankTotal;
  if (SESSION?.startRank) app.rank = SESSION.startRank;
  // 輪胎（第三關移植）：session 可覆寫起始值（週末跨賽段沿用）
  app.tiresMax = SESSION?.tiresMax ?? 5;
  app.tires = Math.max(0, Math.min(app.tiresMax, SESSION?.tiresStart ?? app.tiresMax));
  app.playerLane = 1;
...
function dropCardToStability(cardIdx) {
  ...
  // 規則：每個動作（排位＝一輪手牌）最多丟 3 張進空力；最後一張也能丟
  const STAB_DROP_CAP = 3;
  if (app.stage2 && (app.stage2._stabDrops || 0) >= STAB_DROP_CAP) {
    app.message = `本回合空力已達上限（最多 ${STAB_DROP_CAP} 張）`;
    return false;
  }
  // 處理
  app.hand.splice(cardIdx, 1);
  if (app.stage2) app.stage2._stabDrops = (app.stage2._stabDrops || 0) + 1;
  ...
  // 排位賽：丟掉最後一張手牌 → 自動結束回合、結算當下賽段
  if (isQuali() && app.hand.length === 0) {
...
function playCardToLane(cardIdx, targetLane) {
  ...
  if (isStage2()) {
    ...
    // 處理輪胎消耗（含「保胎策略」的免疫每回合第 1 次扣胎；第三關移植）
    if (card.tireCost) {
      const hasTirePreserve = s2.teamCardsActive.some(c => c.effect === "tirePreserve");
      if (hasTirePreserve && !s2.tirePreserveUsedThisRound) {
        s2.tirePreserveUsedThisRound = true;
    ...
    // tirePreserve（保胎策略）：所有指令牌 -10 速度（第三關移植）
    if (s2.teamCardsActive.some(c => c.effect === "tirePreserve")) {
      playerSpeedSource(-10, "保胎");
    }
...
function proceedAfterCardPlay(card) {
  // 風阻減免：打出後即 arm 尾流加成（本回合吃到尾流時 +bonus、不限是否換道）
  if (card.slipstreamBonusOnLaneChange && app.stage2) {
    app.stage2.dragSlipstreamBonus = card.slipstreamBonusOnLaneChange;
  }
...
function executeOpponentAction(act) {
  if (act.action === "moveTo") {
    ...
    if (act.target === "playerLane") {
      let targetLane = (isStage2() && app.playerLaneBeforeAction != null)
        ? app.playerLaneBeforeAction
        : app.playerLane;
      // c7 避坑（第三關移植）：若目標道是坑、找最近的安全道（清道夫光環免疫、直接撞）
      const potholes = app.stage2?.potholeLanes || [];
...
function pickSmartLaneForOpponent(strategy, bypassAura = false) {
  ...
  if (strategy === "dynamicAvoidOrBlock") {
    ...
    if (slipstreamConsumed) {
      ...
      // c7 坑洞（第三關移植）：若玩家道是坑、改用 avoidPlayer 避坑
      const potholes = app.stage2?.potholeLanes || [];
  ...
  // 計算每條道對手的顯示速度（第三關移植：災害道直接排除）
  const s2h = app.stage2;
  const potholes2 = (s2h && s2h.potholeLanes) ? s2h.potholeLanes : [];
  const hazardLanes = new Set(potholes2);
  if (app.laneBonuses) {
...
function stage2OnRewardPicked(slot) {
  ...
  if (isEquipTeam) {
    ...
    // 裝備時的代價（如：暖胎電熱絲啟動 -1 胎；第三關移植）
    if (picked.costOnEquip?.tire) {
      spendTire(picked.costOnEquip.tire);
      pushSpeedPop("player", `-${picked.costOnEquip.tire} 胎 裝備啟動`, "#ff5fa0");
    }
  } else {
...
function stage2StartNewRound() {
  ...
  s2.tirePreserveUsedThisRound = false;  // 保胎策略：每回合可免疫第 1 次扣胎（第三關移植）
  s2.lastCardType = null;
  s2.lastCardSameStreak = 0;
  ...
  s2.lastActionWasCard = false;
  // 空力區（穩定區）：正賽每回合歸零；排位全程累積、不在此重置（即使走到這裡也守門）
  if (!isQuali()) {
    app.stabilityCharges = app._stabilityFloor || 0;
    app.stabilityDropFx = null;
    if (app.stage2) app.stage2._stabDrops = 0;  // 每回合重置空力棄牌額度
  }
  // 5. 發手牌
  dealStage2Hand();
...
function stage2OnGameWin() {
  app.mode = "all-clear";
  if (SESSION) return;  // 聯賽模式：結果由 sessionComplete 回報、不寫舊進度
  // 記錄第二關通關 → 解鎖第三關（progress.js）
  window.FinalDriverProgress?.markLevelCleared?.(2);
}
// 輪胎歸零：本賽段報銷（聯賽模式以 DNF 結束賽段、單機模式為輸）
function stage2OnTireOut() {
  if (isQuali()) { qualiEnd("tireOut"); return; }  // 排位爆胎 → 排位結束畫面、鎖當前格
  app.mode = "stage2-tire-out";
}
...
function finalizeRhythmFormal() {
  ...
  if (isStage2()) {
    ...
    // 失敗（< 60% 滿分）：扣 1 胎（第三關移植）
    if (!success) {
      spendTire(1);
      pushSpeedPop("player", "-1 胎 超車失誤", "#ff5fa0");
    }
    if (success) {
    ...
    // 守門：若這次 spendTire 已把輪胎扣到 0、輸的畫面優先於結算
...
function handleButton(id) {
  ...
  // 聯賽模式：賽段完成回報 / 練習提前結束
  if (id === "session-complete") {
    if (isQuali()) { qualiComplete(); return; }
    sessionComplete(false); return;
  }
  if (id === "quit-session")     { sessionComplete(true);  return; }
  // 重新來過
...
function update(time) {
  // 排位：玩家被刷後停在賽道播淘汰動畫，到 cutFxUntil 才進結算畫面
  if (isQuali() && app.quali && app.quali.pendingEnd && time >= app.quali.cutFxUntil) {
    const _reason = app.quali.pendingEnd;
    app.quali.pendingEnd = null;
    qualiEnd(_reason);
    return;
  }
  // 車道絲滑移動 lerp
...
function drawInner(time) {
  ...
  // 聯賽模式：自由練習常駐「結束練習」按鈕（隨時可走）
  if (SESSION?.allowQuit && (m === "playing" || m === "prompt-overtake-or-pass")) {
    button("quit-session", "結束練習 ➜", 16, 14, 196, 44, false, "gray");
  }
  ...
  if (m === "stage2-tire-out")    drawStage2TireOutModal();
  if (m === "quali-done") drawQualiDone();
  // 主關卡專屬結算
      ...
      if (!isDragTeamCard && !isOverCancelZone) {
        ...
        if (hoverLane >= 0) {
          ...
          if (li === app.playerLane) {
            ...
            // c7 坑洞警告（第三關移植）
            if (app.stage2?.potholeLanes?.includes(li)) {
...
function drawRace(time) {
  ...
  // ─── c6/c7 hazard sprite：畫在賽道路面上（透視座標；第三關移植）─────
  if (isStage2() && app.stage2 && app.stage2.hazardSpriteProgress != null) {
    const _hzHorizon = h * 0.38;
    const prog = app.stage2.hazardSpriteProgress;
    const spriteY = _hzHorizon + (h - _hzHorizon) * prog;
    const laneCountHz = app.laneCount || 2;
    const persp = 0.25 + prog * 1.0;
    ...
    if (app.stage2.potholeLanes && app.stage2.potholeLanes.length) {
      for (const lane of app.stage2.potholeLanes) {
        const hcx = laneCarX(lane, laneCountHz, spriteY);
        ctx.beginPath();
        ctx.ellipse(hcx, spriteY, rx + outlineW, ry + outlineW, 0, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(255,80,60,0.75)";
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(hcx, spriteY, rx, ry, 0, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(0,0,0,0.88)";
        ctx.fill();
        text("坑", hcx, spriteY + fontSize * 0.35, fontSize, "rgba(255,180,160,0.95)", "900", "center");
      }
    }
    ...
    if (app.laneBonuses) {
      for (const b of app.laneBonuses) {
        ...
        const hcx = laneCarX(b.lane, laneCountHz, spriteY);
        ctx.beginPath();
        ctx.ellipse(hcx, spriteY, rx + outlineW, ry + outlineW, 0, 0, Math.PI * 2);
        ...
        ctx.fill();
        text("油", hcx, spriteY + fontSize * 0.35, fontSize, "rgba(240,180,255,0.95)", "900", "center");
      }
    }
  }
  ...
  if (!isQuali()) drawCar(redX, opponentY, redW, redH, "#e94d48");

  // 對手整合資訊面板（名條 + 預告，合而為一，車子正上方）
  if (isStage2() && app.stage2 && !isQuali()) {
  ...
  if (app.opponentActionFx && time < app.opponentActionFx.until && !isQuali()) {
...
function drawHud(time) {
  ...
  const _rankStr = isQuali() ? `${qualiPlayerRank()} / ${app.quali.totalGrid}` : `${app.rank} / ${app.rankTotal}`;
  text(_rankStr, s.x+s.w-20, s.y+34, 15, "rgba(214,228,255,0.95)", "900", "right");
  hr(s.y+54);
  ...
  const _quali = isQuali();
  const _qCut = _quali ? qualiCutSpeed() : null;
  const nameStr = _quali ? "最高淘汰速度" : (opp?.name || "對手速度");
  const nameColor = _quali ? "rgba(255,170,110,0.9)" : (canOvertakeNow ? "rgba(120,255,160,0.95)" : "rgba(255,160,160,0.85)");
  ...
  const spdColor = _quali ? "rgba(255,175,120,0.95)" : (canOvertakeNow ? "rgba(140,255,180,0.95)" : "rgba(255,150,150,0.95)");
  text(`${_quali ? (_qCut != null ? _qCut : "—") : oppSpd}`, s.x+s.w-20, s.y+148, 26, spdColor, "900", "right");
  if (opp) {
...
// 輪胎健康色（第三關移植：隨胎量 綠 → 黃綠 → 黃 → 橘 → 紅+脈動）
// shared chassis-hud 模組的 tireHealthColor 永遠回傳綠、不用它
function tireHealthColor(time) {
  const pct = app.tires / Math.max(1, app.tiresMax);
  const danger = pct <= 0.25;  // 1/4 以下進入危急
  const pulse = danger ? (0.5 + 0.5 * Math.sin((time || 0) * 0.012)) : 1;
  let main, dim, glow;
  if (pct > 0.7) {
    main = "#5dff7a"; dim = "rgba(80, 200, 110, 0.55)"; glow = "rgba(90, 255, 130, 0.55)";
  } else if (pct > 0.5) {
    main = "#b8ff5d"; dim = "rgba(140, 220, 90, 0.55)"; glow = "rgba(170, 255, 110, 0.55)";
  } else if (pct > 0.3) {
    main = "#ffd94f"; dim = "rgba(220, 180, 90, 0.6)"; glow = "rgba(255, 220, 100, 0.55)";
  } else if (pct > 0.15) {
    main = "#ff8d4f"; dim = "rgba(220, 130, 90, 0.65)"; glow = "rgba(255, 150, 100, 0.6)";
  } else {
    const a = 0.55 + 0.4 * pulse;
    main = `rgba(255, ${Math.round(80 + 40 * (1 - pulse))}, ${Math.round(80 + 40 * (1 - pulse))}, ${a})`;
    dim = `rgba(220, 70, 70, ${0.5 + 0.3 * pulse})`;
    glow = `rgba(255, 90, 90, ${0.4 + 0.5 * pulse})`;
  }
  return { main, dim, glow, danger, pulse };
}

// ── 各部件 metadata（名稱 + 敘述 / 預告）──
const CHASSIS_PARTS = {
  tyres:      { name: "輪胎",         desc: "輪胎歸 0 時車輛報廢、本賽段以 DNF 結束。" },
...
function chassisHoverPart(x, y, w, h) {
  ...
  if (!CH || !CH.chassisHitAreas) return null;
  const areas = CH.chassisHitAreas(x, y, w, h);
...
function drawChassisTooltip(R, partKey) {
  ...
  const ctx = app.ctx;
  // 輪胎部件：標題動態顯示目前耗損層數
  const name = (partKey === "tyres")
    ? `${data.name}　${app.tires} / ${app.tiresMax}`
    : data.name;
...
function drawCarPartsHud(time) {
  ...
  const ctx = app.ctx;
  const CH = window.FinalDriverChassisHud;
  if (!CH || !CH.drawChrome) return;  // shared module 還沒載入、保險不畫

  const hc = tireHealthColor(time);  // 第三關移植：真實胎量色（shared 模組版永遠綠、不用）
  ...
  const stabAtCap = app.stage2 && (app.stage2._stabDrops || 0) >= 3;
  const dragMessage = isDragging
    ? (stabAtCap ? "無法再提升空力（已達 3 張上限）" : "棄手牌至此，提升空力（+1 階、降 QTE 難度）")
    : null;
  ...
  const chrome = CH.drawChrome(ctx, viewport, {
    ...
    tireText: `${app.tires}/${app.tiresMax}`,  // 標題列輪胎狀況
    tireColor: hc.main,
  ...
  // ── 共用：俯視 schematic + 掃描線（都搬到 shared 了）
...
function drawAllClear() {
  ...
  if (SESSION) {
    // 聯賽模式：賽段完成（拿下第 1 名、或練習對手清空）
    const title = SESSION.isMainRace ? "冠軍" : (SESSION.allowQuit ? "練習結束" : "拿下第 1 名");
    text(title, app.w/2, app.h*0.38, 56, "#ffd94f", "1000", "center");
    text(SESSION.name || "", app.w/2, app.h*0.52, 20, "rgba(200,220,255,0.8)", "700", "center");
    button("session-complete", SESSION.completeLabel || "完成賽段 ➜", app.w/2-130, app.h*0.62, 260, 52, false, "start");
    return;
  }
...
function drawStage2SidePanel(time) {
  const s2 = app.stage2;
  if (!s2) return;
  if (isQuali()) { drawQualiBoard(time); return; }  // 排位接手左上名次榜
  const ctx = app.ctx;
  const x = 14;
  const y = 80;
  const w = 240;  // 字體放大後加寬讓標題跟回合計時不重疊
  let curY = y;
  // 面板高度：按實際會顯示的元素動態累加（名次列數、後車警告、尾流、車隊牌）
  const _rankRows = app.rankTotal || 4;
  const rankBlockH = _rankRows * 22 + (_rankRows - 1) * 3;  // 與 drawRankLineup 的 rowH22/gap3 一致
  const _hasChaser = !!currentChaser() && app.rank !== app.rankTotal;
  const _chaserH = _hasChaser ? 54 : 0;
  const _slipH = 22;  // 尾流提示（同道才顯示）；保守保留高度、避免漏估
  const teamCardsH = s2.teamCardsActive.length > 0 ? (8 + 20 + s2.teamCardsActive.length * 22) : 0;
  const panelH = 16 + 34 + 22 + rankBlockH + 14 + _chaserH + _slipH + teamCardsH + 14;
  ...
  for (let pos = 1; pos <= total; pos++) {
    ...
    if (isPlayer) {
      ...
      if (idx >= 0 && idx < s2.ahead.length) {
        fullName = STAGE2_OPPONENTS[s2.ahead[idx]]?.name || qualiDriverName(s2.ahead[idx]);
        color = "#ffb070";
      } else {
        fullName = "?"; color = "rgba(180,140,80,0.5)";
      }
    } else { // pos > playerRank
      // 後方：先填已超越的（passed）、再填原本排在身後的（behind）
      const idx = pos - playerRank - 1;
      let bid = null;
      if (idx >= 0 && idx < s2.passed.length) bid = s2.passed[idx];
      else if (s2.behind && (idx - s2.passed.length) >= 0 && (idx - s2.passed.length) < s2.behind.length) bid = s2.behind[idx - s2.passed.length];
      if (bid) {
        fullName = STAGE2_OPPONENTS[bid]?.name || qualiDriverName(bid);
...
function endBendQte(success) {
  ...
  if (context === "drift") {
    ...
    // 甩尾失敗也耗胎（第三關口徑：QTE 失敗損耗輪胎）
    if (!success) {
      spendTire(1);
      pushSpeedPop("player", "-1 胎 甩尾失誤", "#ff5fa0");
    }
  ...
  app.bendQteResult = { success, mistakeCount, slippedTo, context: "circuit" };
  // 守門：若 spendTire 已把輪胎扣到 0、直接進輸的畫面（第三關移植）
  if (enforceTireOutIfDead()) return;
  app.mode = "bend-qte-result";
  ...
  setTimeout(() => {
    if (app.mode === "bend-qte-result") {
      ...
      // 處理延後的災害位置重抽（QTE 在跑時無法重抽、回到 playing 才重抽；第三關移植）
      else if (app.stage2?._pendingHazardReroll) {
        rerollCircuitHazards(currentCircuit());
        app.stage2._pendingHazardReroll = false;
      }
      app._bendQteContext = null;
      checkAutoPrompt();
    }
...
// 輪胎報銷畫面（第三關移植；聯賽模式：DNF 結束賽段）
function drawStage2TireOutModal() {
  const ctx = app.ctx;
  ...
  if (SESSION) {
    button("session-complete", SESSION.allowQuit ? "結束練習 ➜" : "完成賽段（DNF）➜", cx - 130, cy + 70, 260, 50, false, "primary");
  } else {
...
function drawStage2FinishLineModal() {
  ...
  if (SESSION) {
    button("session-complete", SESSION.completeLabel || "完成賽段 ➜", cx - 130, cy + 130, 260, 50, false, isWin ? "start" : "primary");
  } else {
...
function drawQteDifficultyPanel(qteType) {
  ...
  // 空力區（穩定區）：影響所有 QTE 的 tier（與 speedTierStep 一致）
  const stabCharges = app.stabilityCharges || 0;
  const showStability = stabCharges > 0;
  // Boss 績效考核 Buff/Debuff（同樣影響所有 QTE）
  const bossBuff = app.stage2?.boss?.buffStacks || 0;
  const bossDebuff = app.stage2?.boss?.debuffStacks || 0;
  const showPerfReview = (bossBuff > 0 || bossDebuff > 0);
  ...
  // 動態列：道路節奏（僅超車）/ 空力區 / 績效考核，各佔一行、依序貼齊
  const extraRows = (showQteDiff ? 1 : 0) + (showStability ? 1 : 0) + (showPerfReview ? 1 : 0);
  const panelH = (180 + extraRows * 24 + 14) * UI_SCALE;
  ...
  let nextRowY = panelY + 192 * UI_SCALE;  // 動態列起點：道路節奏／空力／績效依序往下
  ...
  if (showQteDiff) {
    ...
    text(`道路節奏　${diffLabel}`, labelX, nextRowY, 12,
      diffColor, "700", "left");
    nextRowY += 24 * UI_SCALE;
  }
  ...
  if (showStability) {
    text(`✦ 空力區　QTE 難度 -${stabCharges}`,
      labelX, nextRowY, 12, "rgba(140, 255, 160, 0.95)", "800", "left");
    text(`-${stabCharges}`, valueX, nextRowY, 14,
      "rgba(140, 255, 160, 0.95)", "900", "right");
    nextRowY += 24 * UI_SCALE;
  }
  // 第 5 列：Boss 績效考核 Buff/Debuff
  if (showPerfReview) {
    const rowY5 = nextRowY;
...
// ─── 排位榜渲染 ──────────────────────────────────────────────────────
const QUALI_NAMES = { PLAYER:"你", A:"禿鷹", B:"清道夫", C:"破風者", P:"陪跑員", D1:"小路", D2:"阿岩", D3:"千野", D4:"莫克" };
function qualiDriverName(id) { return QUALI_NAMES[id] || id; }

// 進行中側欄即時榜（畫面右側常駐）
function drawQualiBoard(time) {
  const q = app.quali;
  if (!q) return;
  const ctx = app.ctx;
  const x = 14, y = 80, w = 240;
  // 外框：沿用舊名次面板風格（roundPanel + 橙色賽道標題 + 回合資訊）
  const rowH = 26, gap = 4;
  const headH = 78;
  const panelH = headH + q.drivers.length * (rowH + gap) + 14;
  roundPanel(x, y, w, panelH, 12, "rgba(10,18,28,0.9)", "rgba(120,170,220,0.4)", 1.5);
  text("霓虹城市賽道", x + 14, y + 24, 17, "rgba(255,220,120,0.85)", "900");
  const isFinalLap = q.lap >= q.laps;
  // 標題行右：圈數
  text(q.finished ? "已完成" : `${q.lap} / ${q.laps} 圈`, x + w - 14, y + 24, 13, "rgba(180,200,230,0.85)", "800", "right");
  // 第二行（原最高淘汰速度位置）：淘汰倒數
  const segsLeft = Math.max(1, q.segsPerLap - q.seg + 1);
  const countdownTxt = q.finished ? "起跑順位已確定"
    : isFinalLap ? "最後一圈・全員鎖定中"
    : `再 ${segsLeft} 段淘汰`;
  const countdownColor = (!q.finished && !isFinalLap && segsLeft <= 2) ? "rgba(255,170,90,0.98)" : "rgba(180,205,235,0.85)";
  text(countdownTxt, x + 14, y + 50, 13, countdownColor, "800");
  // 行繪製用：存活車排序（危險線判定）
  const aliveSorted = qualiAlive().slice().sort((a, b) => b.total - a.total);
  const aliveN = aliveSorted.length;
  text("排位名次", x + 14, y + 70, 11, "rgba(160,190,230,0.6)", "700");
  // 排序檢視：已鎖格排自己位、未鎖依 total
  const view = q.drivers.slice().sort((a, b) => {
    if (a.gridPos !== null && b.gridPos !== null) return a.gridPos - b.gridPos;
    if (a.gridPos !== null) return 1;
    if (b.gridPos !== null) return -1;
    return b.total - a.total;
  });
  const dangerLine = aliveN - q.cut;
  const listY = y + headH;
  let aliveIdx = 0;
  view.forEach((d, i) => {
    const rowY = listY + i * (rowH + gap);
    const locked = d.gridPos !== null;
    const danger = !locked && (q.lap < q.laps) && (aliveIdx >= dangerLine);
    const justCut = q.lastCutIds.includes(d.id) && performance.now() < q.cutFxUntil;
    // 底色：玩家綠調、鎖定灰、危險橙、剛淘汰紅
    let fill = d.isPlayer ? "rgba(60,100,80,0.45)" : "rgba(40,55,80,0.45)";
    if (locked) fill = "rgba(44,48,56,0.5)";
    if (justCut) fill = "rgba(150,50,50,0.55)";
    else if (danger) fill = "rgba(120,75,30,0.5)";
    // 邊框色
    let stroke = d.isPlayer ? "#7be0a0" : "rgba(120,150,190,0.5)";
    if (locked) stroke = "rgba(150,160,175,0.5)";
    if (danger) stroke = "rgba(255,170,90,0.8)";
    if (justCut) stroke = "rgba(255,120,120,0.9)";
    ctx.fillStyle = fill;
    ctx.fillRect(x + 12, rowY, w - 24, rowH);
    ctx.strokeStyle = stroke;
    ctx.lineWidth = d.isPlayer ? 2 : 1;
    ctx.strokeRect(x + 12.5, rowY + 0.5, w - 25, rowH - 1);
    const ty = rowY + rowH/2 + 4;
    const posLabel = locked ? `P${d.gridPos}` : `${aliveIdx + 1}`;
    const posColor = locked ? "rgba(255,180,90,0.95)" : (d.isPlayer ? "#9ff0bb" : "rgba(180,200,230,0.7)");
    text(posLabel, x + 22, ty, 12, posColor, "900", "left");
    text(qualiDriverName(d.id), x + 50, ty, 12, d.isPlayer ? "#fff" : (locked ? "rgba(190,200,215,0.8)" : "rgba(220,232,250,0.92)"), "800", "left");
    // 右欄：鎖定→🔒、危險→⚠、否則累積分
    let rstr, rcolor;
    if (locked) { rstr = "🔒"; rcolor = "rgba(255,180,90,0.9)"; }
    else if (justCut) { rstr = "✕ 淘汰"; rcolor = "#ffb0b0"; }
    else { rstr = `${d.total}`; rcolor = d.isPlayer ? "#9ff0bb" : "rgba(200,225,255,0.9)"; }
    text(rstr, x + w - 22, ty, 12, rcolor, "900", "right");
    if (!locked) aliveIdx++;
  });
}

function drawQualiDone() {
  drawLanes(performance.now());
  const ctx = app.ctx;
  ctx.fillStyle = "rgba(0,0,0,0.6)";
  ctx.fillRect(0, 0, app.w, app.h);
  drawQualiBoard(performance.now());  // 黑底之後畫、最終榜清楚顯示
  const q = app.quali;
  const cx = app.w * 0.5, cy = app.h * 0.42;
  const reasonTxt = q.endReason === "tireOut" ? "輪胎報銷 — 排位提前結束"
    : q.endReason === "cut" ? "你被淘汰了" : "排位賽完成";
  text(reasonTxt, cx, cy - 60, 34, q.endReason === "finish" ? "#bcd8ff" : "#ffb060", "1000", "center");
  text(`你的起跑順位　P${q.playerGridPos}`, cx, cy + 6, 44, "#ffe08a", "1000", "center");
  text("接下來的比賽將從此順位開始。", cx, cy + 52, 16, "rgba(200,220,250,0.85)", "700", "center");
  button("session-complete", "前往正賽 ➜", cx - 140, cy + 100, 280, 54, false, "start");
}
...
// ─── 聯賽（賽事週末）Session 層 ─────────────────────────────────────────
// run.js 透過 startSessionRace(cfg) 啟動一個賽段、cfg.onComplete(result) 收結果。
// cfg: { id, name, lineup, tutorial, maxRounds, allowQuit, deckPermanent,
//        startRank, rankTotal, buffs, isMainRace, completeLabel, onComplete }
// ═══════════════════════════════════════════════════════════════════════
// 排位賽（無對手・計時實跑）
//   玩家真的在賽道上跑 N 段（每張牌＝1 段、牌效全生效、賽段 QTE 照常、耗胎照常）；
//   8 台 AI 在側欄榜以 base±jitter 每段累積；每圈刷墊底 cutPerLap 台、鎖起跑格。
//   玩家被刷 / 爆胎 → 立刻結束實跑、鎖當前墊底格、AI 快轉補完順位。
// ═══════════════════════════════════════════════════════════════════════
function isQuali() { return !!(SESSION && SESSION.qualiMode); }

function initQualiBoard() {
  const drivers = [];
  for (const r of WEEKEND_ROSTER) {
    drivers.push({
      id: r.id, isPlayer: !!r.isPlayer,
      base: r.qualiBase || 0, jitter: r.qualiJitter || 0,
      total: 0, lastGain: null, gridPos: null,
    });
  }
  app.quali = {
    drivers,
    laps: QUALI_CONFIG.laps, segsPerLap: QUALI_CONFIG.segmentsPerLap, cut: QUALI_CONFIG.cutPerLap,
    lap: 1, seg: 1, totalGrid: drivers.length, nextLockSlot: drivers.length,
    phase: "running", lastCutIds: [], finished: false, gridOrder: null,
    segStartSpeed: 0, cutFxUntil: 0, pendingEnd: null,
  };
}

function qualiAlive() { return app.quali.drivers.filter(d => d.gridPos === null); }
function qualiPlayer() { return app.quali.drivers.find(d => d.isPlayer); }
function qualiAiGain(d) {
  const lapBonus = (app.quali.lap - 1) * 5;  // 每過一圈、所有 AI base +5（隨賽程提速）
  return Math.max(0, Math.round(d.base + lapBonus + (Math.random() * 2 - 1) * d.jitter));
}
// 結算一段：玩家用實跑累積速度（total 直接讀 playerSpeed）、AI 各自 +gain
function qualiResolveSegment() {
  const q = app.quali;
  for (const d of qualiAlive()) {
    if (d.isPlayer) {
      const cur = Math.round(app.playerSpeed);  // 圈內累積速度（不重置）
      d.lastGain = cur - d.total;
      d.total = cur;
    } else {
      d.lastGain = qualiAiGain(d);
      d.total += d.lastGain;
    }
  }
}
// 跑完一圈 → 刷墊底 cut 台（最後一圈＝鎖定剩餘全部名次）
function qualiDoCut() {
  const q = app.quali;
  const isFinalLap = (q.lap >= q.laps);
  const sorted = qualiAlive().slice().sort((a, b) => b.total - a.total);
  if (isFinalLap) {
    sorted.forEach((d, i) => { d.gridPos = i + 1; });
    q.lastCutIds = [];
  } else {
    const cut = sorted.slice(-q.cut);
    q.lastCutIds = cut.map(d => d.id);
    const slots = [];
    for (let i = q.cut - 1; i >= 0; i--) slots.push(q.nextLockSlot - i);
    cut.forEach((d, i) => { d.gridPos = slots[i]; });
    q.nextLockSlot -= q.cut;
  }
  q.cutFxUntil = performance.now() + 1400;
}
// 玩家被刷 / 爆胎後：快轉剩餘段、AI 自動補完所有起跑格
function qualiFastForward() {
  const q = app.quali;
  let guard = 0;
  while (qualiAlive().length && guard++ < 200) {
    if (q.seg > q.segsPerLap) { qualiDoCut(); q.lap += 1; q.seg = 1; continue; }
    // 玩家已鎖格時、resolveSegment 只動 AI
    for (const d of qualiAlive()) { if (!d.isPlayer) { d.lastGain = qualiAiGain(d); d.total += d.lastGain; } }
    q.seg += 1;
    if (q.seg > q.segsPerLap && q.lap >= q.laps) { qualiDoCut(); break; }
  }
  // 保險：仍有未鎖格者、依 total 收尾
  if (qualiAlive().length) qualiDoCut();
}
function qualiBuildGridOrder() {
  return app.quali.drivers.slice().sort((a, b) => (a.gridPos || 99) - (b.gridPos || 99)).map(d => d.id);
}
// 最高淘汰速度：存活車排序後、會被刷的那群裡最高的分數（玩家超過它才安全）
// 玩家即時名次（未鎖定→存活排序中位置；已鎖定→格位）
function qualiPlayerRank() {
  const q = app.quali;
  const p = qualiPlayer();
  if (!p) return 1;
  if (p.gridPos) return p.gridPos;
  const view = q.drivers.slice().sort((a, b) => {
    if (a.gridPos !== null && b.gridPos !== null) return a.gridPos - b.gridPos;
    if (a.gridPos !== null) return 1;
    if (b.gridPos !== null) return -1;
  ...
  return view.findIndex(d => d.isPlayer) + 1;
}
function qualiCutSpeed() {
  const q = app.quali;
  if (!q || q.finished || q.lap >= q.laps) return null;
  const sorted = qualiAlive().slice().sort((a, b) => b.total - a.total);
  if (sorted.length <= q.cut) return null;
  return sorted[sorted.length - q.cut]?.total ?? null;
}

// 每圈淘汰後：存活車速度全部回到起始值 10、總分歸零，下一圈重新累積
const QUALI_START_SPEED = 10;
function qualiResetLap() {
  app.playerSpeed = QUALI_START_SPEED;
  for (const d of qualiAlive()) {
    d.total = d.isPlayer ? QUALI_START_SPEED : d.base;  // AI 起始＝base
    d.lastGain = null;
  }
}
// 每跑完 1 段（每張牌切段後）呼叫：餵榜、計圈、判淘汰 / 爆胎 / 結束
function qualiAfterSegment() {
  const q = app.quali;
  if (!q || q.finished) return;
  // 爆胎：立刻收尾（鎖當前墊底格）
  if (app.tires <= 0) { qualiEnd("tireOut"); return; }
  qualiResolveSegment();
  q.seg += 1;
  // 一圈跑完（7 賽段）→ 淘汰 / 完賽
  if (q.seg > q.segsPerLap) {
    const wasFinalLap = (q.lap >= q.laps);
    qualiDoCut();
    if (wasFinalLap) { qualiEnd("finish"); return; }  // 最後一圈完賽（doCut 已鎖全部）
    if (qualiPlayer().gridPos) {
      // 玩家被刷：先停在賽道播 1.4 秒淘汰動畫（名次榜已寫入最終速度、你那行會標紅「✕ 淘汰」），
      // 動畫播完才由 update 的 tick 進結算畫面 → 玩家看得到「我最後一段有算、只是被 AI 擠下去」
      q.pendingEnd = "cut";
      app.inputLocked = true;  // cutFxUntil 期間鎖輸入；tickSpeedPopGates 因 pendingEnd 不解鎖
      return;
    }
    q.lap += 1; q.seg = 1;
    qualiResetLap();  // 淘汰後：存活車速度歸零、重新累積
  }
  // 手牌全部打完才補回 5 張；補牌時重置空力 charge ＋ 棄牌額度（一輪手牌週期）
  if (!app.hand || app.hand.length === 0) {
    dealStage2Hand();
    app.stabilityCharges = app._stabilityFloor || 0;
    if (app.stage2) app.stage2._stabDrops = 0;
  }
}

// 結束排位：補完順位、進結算畫面
function qualiEnd(reason) {
  const q = app.quali;
  if (q.finished) return;
  q.finished = true;
  q.endReason = reason;
  // 還有車沒鎖格（玩家被刷時、晉級車尚未定位 / 爆胎）→ 快轉補完全部名次
  if (qualiAlive().length) qualiFastForward();
  q.gridOrder = qualiBuildGridOrder();
  q.playerGridPos = qualiPlayer().gridPos;
  app.mode = "quali-done";
}

let SESSION = null;

function applySessionBuffs() {
  const s2 = app.stage2;
  if (!s2 || !SESSION?.buffs) return;
  for (const b of SESSION.buffs) {
    if (b === "warmTires") {
      // 復用 newTireWarm 的 keepSpeed 效果（裝備牌、永久）
      s2.teamCardsActive.push({ type:"buffWarmTires", cardClass:"team", name:"暖胎完成",
        note:"每回合結算後保留 +10 速度", effect:"keepSpeed", value:10,
        trigger:"equip", persistence:"permanent", persistenceLabel:"進場 buff", color:"team", id:"buff-warm" });
    } else if (b === "momentum") {
      // 氣勢如虹：每回合開局自帶 1 格穩定區（stage2StartNewRound 重置時讀地板值）
      app._stabilityFloor = 1;
      if ((app.stabilityCharges || 0) < 1) app.stabilityCharges = 1;
    } else if (b === "intel") {
      // 復用 bigData 的 showOpponent 效果
      s2.teamCardsActive.push({ type:"buffIntel", cardClass:"team", name:"賽前情報",
        note:"顯示對手下一招的具體內容", effect:"showOpponent", value:1,
        trigger:"equip", persistence:"permanent", persistenceLabel:"進場 buff", color:"team", id:"buff-intel" });
    }
  }
}

function buildSessionResult(skipped) {
  const s2 = app.stage2;
  return {
    sessionId: SESSION?.id || null,
    skipped: !!skipped,
    rank: app.rank,
    rankTotal: app.rankTotal,
    rounds: s2 ? (s2.roundsPlayed || 0) : 0,
    // 輪胎（第三關移植）：剩餘胎量＋是否爆胎 DNF（run 層接走、跨賽段沿用）
    tires: app.tires,
    tireOut: app.mode === "stage2-tire-out",
    // 本賽段累積到的永久牌（含三選一拿的）— run 層接走、下一段沿用
    deckPermanent: s2 ? s2.deckPermanent.map(c => ({...c})) : [],
    // 終點時仍在玩家前方的車手（含當前對手）— 經營層判「有沒有贏過對手車隊」用
    aheadIds: s2 ? [ ...(s2.currentOpponentId ? [s2.currentOpponentId] : []), ...((s2.ahead)||[]) ] : [],
  };
}

// 排位結束回報：把 gridOrder 交給 run 層
function qualiComplete() {
  const order = app.quali?.gridOrder || qualiBuildGridOrder();
  const circuitOrder = app.quali?.circuitOrder || null;
  const cb = SESSION?.onComplete;
  SESSION = null;
  app._stabilityFloor = 0;
  stopNormalBgm();
  app.mode = "session-idle";
  if (typeof cb === "function") cb(order, circuitOrder);
}
function sessionComplete(skipped) {
  const result = buildSessionResult(skipped);
  const cb = SESSION?.onComplete;
  SESSION = null;             // 先清、避免 callback 內重入
  app._stabilityFloor = 0;
  stopNormalBgm();
  app.mode = "session-idle";  // 引擎待機、畫面交還 shell
  if (typeof cb === "function") cb(result);
}

// 排位載入後：建榜、改用排位賽段池、發整包指令卡、每段強制 1 動作切段
function initQualiAfterLoad() {
  initQualiBoard();
  const s2 = app.stage2;
  if (!s2) return;
  s2.ahead = [];
  s2.currentOpponentId = null;
  s2.chaserId = null;
  s2.tutorial = { active: false, stepIndex: 0, stepShownAt: performance.now() };
  let pool;
  if (SESSION?.circuitOrder && SESSION.circuitOrder.length) {
    pool = [...SESSION.circuitOrder];  // 沿用 run 層決定的賽道順序
  } else {
    pool = [...QUALI_CIRCUIT_POOL];
    for (let i = pool.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [pool[i], pool[j]] = [pool[j], pool[i]];
    }
  }
  app.quali.circuitOrder = [...pool];  // 回報給 run 層、之後比賽沿用
  s2.circuitOrder = [...pool, ...pool, ...pool];
  s2.circuitIndex = s2.circuitOrder[0];
  applyCircuit(STAGE2_CIRCUITS[s2.circuitIndex]);
  s2.circuitStepsLeft = 1;
  s2.firstRoundReady = false;
  s2.drawPile = buildQualiHand();  // 12 張牌庫（牌庫去失誤＋標準指令補滿）
  shuffleArrayInPlace(s2.drawPile);
  s2.discardPile = [];
  app.hand = [];
  dealStage2Hand();  // 發 5 張手牌（之後每段打 1 補 1、整副循環）
  app.playerSpeed = QUALI_START_SPEED;  // 起始速度（每圈淘汰後也回到這）
  qualiResetLap();  // 玩家 total=10、AI total=各自 base
  app.mode = "playing";
}
function buildQualiHand() {
  const POOL_SIZE = 12;
  const out = [];
  const base = makeStage2InitialDeck();
  const perm = (SESSION?.deckPermanent || []).map(c => ({ ...c }));
  for (const c of [...base, ...perm]) {
    const def = STAGE2_ALL_CARDS[c.type];
    if (!def || def.cardClass !== "action" || c.type === "mistake") continue;
    out.push(c);
  }
  let fillN = 0;
  while (out.length < POOL_SIZE) out.push(makeCard("standard", `quali-std-${fillN++}`));
  return out.slice(0, POOL_SIZE);
}
function startSessionRace(cfg) {
  SESSION = cfg || {};
  app._stabilityFloor = 0;
  reset();             // 清掉上一賽段殘留（QTE 集合、拖曳、防守旗標…）
  SESSION = cfg || {}; // reset() 不動 SESSION、但保險重設一次
  playNormalBgm();
  loadStage(0);
  applySessionBuffs();
  if (isQuali()) initQualiAfterLoad();
}

export { start, startSessionRace };
