(function () {
  ...
  function drawChrome(ctx, viewport, options = {}) {
    ...
    // 3) 標題列（綠字）+ 右上角充能數
    // 標題列：有輪胎資訊就顯示「輪胎 X/Y · CHASSIS」（胎色），否則原車體狀態字樣
    const titleText = options.tireText
      ? `\u8f2a\u80ce ${options.tireText}`
      : "\u8eca\u9ad4\u72c0\u614b";
    const titleColor = options.tireColor || "rgba(93, 255, 122, 0.78)";
    text(ctx, titleText,
         rect.x + 14, rect.y + 22, 12, titleColor, "800");
    text(ctx, `\u7a7a\u529b ${charges}/3`,
