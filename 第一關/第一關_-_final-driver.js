...
<div id="qteWinOverlay" class="qte-win-overlay hidden" aria-hidden="true">
  ...
  function update(time) {
    if (isRhythmMode()) {
      for (let i = 0; i < 5; i++) {
        ...
        if (time >= judgeT) {
          app.qteFinalized[i] = true;
          let outcome = rhythmOutcomeFromTap(undefined, start, dur, judgeT);
          if (outcome === "miss" && app.perkPressure) app.pressureWideBand = true;
          app.qteResults[i] = outcome;
          app.qteDismissAt[i] = time + 1000;
          app.qteClicked.add(i);
  ...
  function drawRhythm(time) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // 判定結果文字：顯示在圓圈外部的底下（與第二關一致）
      if (displayOutcome) {
        const txtCol = displayOutcome === "perfect" ? "#a6efb6"
                     : displayOutcome === "good"    ? "#ffe89a"
                     :                                "#ff9aa6";
        const label = displayOutcome === "perfect" ? "Perfect"
                    : displayOutcome === "good"    ? "Good"
                    :                                "Miss";
        text(label, x, y + RHYTHM_OUTER_R + 22, 16, txtCol, "900", "center");
      }
  ...
  function drawQteRhythmOutcomeDisk(ctx, x, y, outerR, grade) {
    ...
    if (grade === "perfect" || grade === "hit") {
      fill = "rgba(130,235,150,0.32)";
      stroke = "rgba(160,245,180,0.95)";
    } else if (grade === "good") {
      fill = "rgba(255,228,120,0.30)";
      stroke = "rgba(255,235,150,0.95)";
    } else {
      fill = "rgba(255,120,130,0.30)";
      stroke = "rgba(255,160,170,0.95)";
    }
    ctx.beginPath();
    ctx.arc(x, y, outerR, 0, Math.PI * 2);
    ctx.fillStyle = fill;
